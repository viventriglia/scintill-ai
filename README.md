...
- [SuperMAG](https://supermag.jhuapl.edu/mag/?fidelity=low&start=2024-01-01T00%3A00%3A00.000Z&interval=23%3A59): ground-based magnetometers in Kourou (KOU, French Guiana) and Tatuoca (TTB, Brazil); unlike INTERMAGNET, it is possible to download partially processed data with [baseline removal](https://agupubs.onlinelibrary.wiley.com/doi/10.1029/2012JA017683). Data can be downloaded manually in bulk for all stations (year by year) from the website. Once downloaded, simply extract the files and place them under the `./data/in/supermag` directory; the data can then be ingested and processed via the `scintill_ai.io` module.

- [ISMR](https://ismrquerytool.fct.unesp.br/is/index.php#): GNSS receiver in Presidente Prudente (PRU2, Brazil). The access to the API is exclusive for authorized researchers and a unique key is required. Your key is provided when you access [the webservice](https://ismrquerytool.fct.unesp.br/is/ismrtool/manual/mkdocs-ismrtools/webservice/); once you have it, place it in the `.env.shared` file and rename it to `.env.secret`. Data can then be downloaded asynchronously via the `scintill_ai.io_async` module, which performs at most `MAX_CONCURRENT_REQUESTS` concurrent requests.
